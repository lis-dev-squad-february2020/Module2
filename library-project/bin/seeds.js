...
const DB_TITLE = 'library-project';
...
// 1. Create authors
//
// Extract a flat array of authors from data
let authors = [];
data.forEach(book => {
  authors = authors.concat(book.author);
});
Author.create(authors)
  .then(authors => {
    ...
    //
    //
    const authorsIds = authors.map(author => author.id); // ['5dee2b9d74cfd697e7a5a0d3', '5dee2b9d74cfd697e7a5a0d4', ...]
    let nth = 0;
    // In each book, replace each author object by it's ID (created in 1.)
    const books = data.map(book => {
      book.author.forEach((author, i) => {
        book.author[i] = authorsIds[++nth];
      });
      return book;
    });
    Book.create(books)
